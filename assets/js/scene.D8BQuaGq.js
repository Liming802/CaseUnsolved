import{R as S,V as A,S as L,P,W as D,O as E,a as O,b as R,C as z,M as p,c as q,d as j,e as w,B as N,f as T,D as b,g as B,h as d,T as M,i as $,E as x,Q as u,j as f}from"./three.DHARaMpq.js";function V(){return console.log("检查THREE.js加载状态"),console.log("THREE对象类型:",typeof M),console.log("THREE版本:",$),console.log("OrbitControls类型:",typeof E),typeof M!="object"?(console.error("THREE.js未正确加载!"),!1):typeof E!="function"?(console.error("OrbitControls未正确加载!"),!1):(console.log("THREE.js和控制组件加载正常"),!0)}class I{constructor(e){this.camera=e,this.enabled=!0,this.deviceOrientation={},this.screenOrientation=0,this.alphaOffset=0,this.smoothingFactor=.2,this.currentRotation=new x,this.targetRotation=new x,this.currentQuaternion=new u,this.targetQuaternion=new u,this.onDeviceOrientationChangeEvent=this.onDeviceOrientationChangeEvent.bind(this),this.onScreenOrientationChangeEvent=this.onScreenOrientationChangeEvent.bind(this),this.connect()}onDeviceOrientationChangeEvent(e){this.deviceOrientation=e}onScreenOrientationChangeEvent(){this.screenOrientation=window.orientation||0}connect(){this.onScreenOrientationChangeEvent(),window.addEventListener("orientationchange",this.onScreenOrientationChangeEvent,!1),window.addEventListener("deviceorientation",this.onDeviceOrientationChangeEvent,!1),this.enabled=!0}disconnect(){window.removeEventListener("orientationchange",this.onScreenOrientationChangeEvent,!1),window.removeEventListener("deviceorientation",this.onDeviceOrientationChangeEvent,!1),this.enabled=!1}update(){if(!this.enabled)return;const e=this.deviceOrientation;if(e){const i=e.alpha?f.degToRad(e.alpha)+this.alphaOffset:0,t=e.beta?f.degToRad(e.beta):0,n=e.gamma?f.degToRad(e.gamma):0,a=this.screenOrientation?f.degToRad(this.screenOrientation):0;this.updateCamera(i,t,n,a)}}updateCamera(e,i,t,n){const a=new x;e=e*.75,i=i*.75,t=t*.75,a.set(-i,-e,t,"YXZ"),this.targetQuaternion.setFromEuler(a),this.targetQuaternion.multiply(new u().setFromAxisAngle(new d(0,0,1),n)),this.targetQuaternion.multiply(new u().setFromAxisAngle(new d(1,0,0),Math.PI/2)),u.slerp(this.camera.quaternion,this.targetQuaternion,this.currentQuaternion,this.smoothingFactor),this.camera.quaternion.copy(this.currentQuaternion)}updateAlphaOffset(){this.deviceOrientation&&this.deviceOrientation.alpha&&(this.alphaOffset=Math.PI/2-f.degToRad(this.deviceOrientation.alpha))}calibrate(){this.updateAlphaOffset()}}class F{constructor(){if(this.markers=[],this.raycaster=new S,this.mouse=new A,this.isMobile=this.checkIfMobile(),this.hasDeviceOrientation=!1,this.controls=null,this.deviceControls=null,console.log("Initializing PanoramaScene... Mobile device:",this.isMobile),!V()){console.error("THREE.js 初始化失败，将在5秒后重试"),setTimeout(()=>this.initialize(),5e3);return}this.initialize()}checkIfMobile(){return/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}initialize(){try{console.log("开始初始化PanoramaScene组件"),this.scene=new L,this.camera=new P(75,window.innerWidth/window.innerHeight,.1,1e3),this.camera.position.set(0,.03,-.1),this.camera.rotation.set(0,Math.PI,0),this.renderer=new D({antialias:!0}),this.renderer.setSize(window.innerWidth,window.innerHeight),this.renderer.setPixelRatio(window.devicePixelRatio),this.renderer.domElement.style.position="fixed",this.renderer.domElement.style.top="0",this.renderer.domElement.style.left="0",this.renderer.domElement.style.width="100%",this.renderer.domElement.style.height="100%",this.renderer.domElement.style.zIndex="5",this.renderer.domElement.style.display="none";const e=document.querySelector(".background-container");e?(e.appendChild(this.renderer.domElement),console.log("Renderer added to background container")):(document.body.appendChild(this.renderer.domElement),console.error("背景容器未找到，renderer添加到body")),this.initControls(),this.renderer.domElement.addEventListener("wheel",i=>{i.preventDefault()},{passive:!1}),window.addEventListener("resize",this.onWindowResize.bind(this)),this.renderer.domElement.addEventListener("click",this.onMarkerClick.bind(this)),this.createVignette(),this.animate(),console.log("PanoramaScene initialized successfully")}catch(e){console.error("PanoramaScene初始化失败:",e)}}initControls(){this.controls=new E(this.camera,this.renderer.domElement),this.setupOrbitControls(this.controls),this.isMobile?(console.log("移动设备：初始化设备方向控制器"),this.deviceControls=new I(this.camera),this.requestDeviceOrientationPermission()):console.log("桌面设备：使用轨道控制器")}setupOrbitControls(e){e.enableZoom=!0,e.zoomSpeed=.5,e.minDistance=.1,e.maxDistance=.3,e.enableDamping=!0,e.dampingFactor=.05,e.rotateSpeed=.5,e.minPolarAngle=Math.PI/3,e.maxPolarAngle=2*Math.PI/3,e.enablePan=!1}requestDeviceOrientationPermission(){if(typeof DeviceOrientationEvent<"u"&&typeof DeviceOrientationEvent.requestPermission=="function"){const e=document.createElement("button");e.style.cssText=`
                ...
                z-index: 10000;
                cursor: default;
            `,document.body.appendChild(e),e.addEventListener("click",()=>{DeviceOrientationEvent.requestPermission().then(i=>{i==="granted"&&this.enableDeviceOrientation(),e.remove()}).catch(()=>{e.remove()})}),e.click(),setTimeout(()=>{document.body.contains(e)&&e.remove()},5e3)}else this.enableDeviceOrientation()}enableDeviceOrientation(){let e,i=!1;const t=n=>{(n.alpha!==null||n.beta!==null||n.gamma!==null)&&(i=!0,clearTimeout(e),window.removeEventListener("deviceorientation",t),this.hasDeviceOrientation=!0,this.deviceControls?(this.deviceControls.disconnect(),this.deviceControls.connect(),this.deviceControls.calibrate()):this.deviceControls=new I(this.camera),this.controls&&(this.controls.enabled=!1))};window.addEventListener("deviceorientation",t),e=setTimeout(()=>{i||(window.removeEventListener("deviceorientation",t),this.controls&&(this.controls.enabled=!0))},2e3)}showFallbackMessage(){}createVignette(){const e=new O(2,2),i=new R({uniforms:{color:{value:new z(0)},opacity:{value:.5}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
        ...
        `;const o=n.investigationCost||{type:"tech",amount:3},r=!a&&((C=window.detectiveManager)==null?void 0:C.hasEnoughPoints(o.type.toLowerCase(),o.amount));s.innerHTML=`
