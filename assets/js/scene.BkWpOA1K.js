import{R as S,V as A,S as L,P,W as D,O as E,a as O,b as R,C as z,M as p,c as q,d as j,e as w,B as N,f as T,D as b,g as B,h as d,T as M,i as $,E as x,Q as u,j as f}from"./three.DHARaMpq.js";function V(){return console.log("检查THREE.js加载状态"),console.log("THREE对象类型:",typeof M),console.log("THREE版本:",$),console.log("OrbitControls类型:",typeof E),typeof M!="object"?(console.error("THREE.js未正确加载!"),!1):typeof E!="function"?(console.error("OrbitControls未正确加载!"),!1):(console.log("THREE.js和控制组件加载正常"),!0)}class I{constructor(e){this.camera=e,this.enabled=!0,this.deviceOrientation={},this.screenOrientation=0,this.alphaOffset=0,this.smoothingFactor=.2,this.currentRotation=new x,this.targetRotation=new x,this.currentQuaternion=new u,this.targetQuaternion=new u,this.onDeviceOrientationChangeEvent=this.onDeviceOrientationChangeEvent.bind(this),this.onScreenOrientationChangeEvent=this.onScreenOrientationChangeEvent.bind(this),this.connect()}onDeviceOrientationChangeEvent(e){this.deviceOrientation=e}onScreenOrientationChangeEvent(){this.screenOrientation=window.orientation||0}connect(){this.onScreenOrientationChangeEvent(),window.addEventListener("orientationchange",this.onScreenOrientationChangeEvent,!1),window.addEventListener("deviceorientation",this.onDeviceOrientationChangeEvent,!1),this.enabled=!0}disconnect(){window.removeEventListener("orientationchange",this.onScreenOrientationChangeEvent,!1),window.removeEventListener("deviceorientation",this.onDeviceOrientationChangeEvent,!1),this.enabled=!1}update(){if(!this.enabled)return;const e=this.deviceOrientation;if(e){const i=e.alpha?f.degToRad(e.alpha)+this.alphaOffset:0,n=e.beta?f.degToRad(e.beta):0,t=e.gamma?f.degToRad(e.gamma):0,a=this.screenOrientation?f.degToRad(this.screenOrientation):0;this.updateCamera(i,n,t,a)}}updateCamera(e,i,n,t){const a=new x,s=new u;e*=.5,i*=.5,n*=.5,a.set(i,e,-n,"YXZ"),s.setFromEuler(a),s.multiply(new u().setFromAxisAngle(new d(0,0,1),t)),s.multiply(new u().setFromAxisAngle(new d(1,0,0),Math.PI/2)),this.camera.quaternion.slerp(s,.2)}updateAlphaOffset(){this.deviceOrientation&&this.deviceOrientation.alpha&&(this.alphaOffset=Math.PI/2-f.degToRad(this.deviceOrientation.alpha))}calibrate(){this.updateAlphaOffset()}}class F{constructor(){if(this.markers=[],this.raycaster=new S,this.mouse=new A,this.isMobile=this.checkIfMobile(),this.hasDeviceOrientation=!1,this.controls=null,this.deviceControls=null,console.log("Initializing PanoramaScene... Mobile device:",this.isMobile),!V()){console.error("THREE.js 初始化失败，将在5秒后重试"),setTimeout(()=>this.initialize(),5e3);return}this.initialize()}checkIfMobile(){return/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}initialize(){try{console.log("开始初始化PanoramaScene组件"),this.scene=new L,this.camera=new P(75,window.innerWidth/window.innerHeight,.1,1e3),this.camera.position.set(0,.03,-.1),this.camera.rotation.set(0,Math.PI,0),this.renderer=new D({antialias:!0}),this.renderer.setSize(window.innerWidth,window.innerHeight),this.renderer.setPixelRatio(window.devicePixelRatio),this.renderer.domElement.style.position="fixed",this.renderer.domElement.style.top="0",this.renderer.domElement.style.left="0",this.renderer.domElement.style.width="100%",this.renderer.domElement.style.height="100%",this.renderer.domElement.style.zIndex="5",this.renderer.domElement.style.display="none";const e=document.querySelector(".background-container");e?(e.appendChild(this.renderer.domElement),console.log("Renderer added to background container")):(document.body.appendChild(this.renderer.domElement),console.error("背景容器未找到，renderer添加到body")),this.initControls(),this.renderer.domElement.addEventListener("wheel",i=>{i.preventDefault()},{passive:!1}),window.addEventListener("resize",this.onWindowResize.bind(this)),this.renderer.domElement.addEventListener("click",this.onMarkerClick.bind(this)),this.createVignette(),this.animate(),console.log("PanoramaScene initialized successfully")}catch(e){console.error("PanoramaScene初始化失败:",e)}}initControls(){this.controls=new E(this.camera,this.renderer.domElement),this.setupOrbitControls(this.controls),this.isMobile?(console.log("移动设备：初始化设备方向控制器"),this.deviceControls=new I(this.camera),this.requestDeviceOrientationPermission()):console.log("桌面设备：使用轨道控制器")}setupOrbitControls(e){e.enableZoom=!0,e.zoomSpeed=.5,e.minDistance=.1,e.maxDistance=.3,e.enableDamping=!0,e.dampingFactor=.05,e.rotateSpeed=.5,e.minPolarAngle=Math.PI/3,e.maxPolarAngle=2*Math.PI/3,e.enablePan=!1}requestDeviceOrientationPermission(){if(typeof DeviceOrientationEvent<"u"&&typeof DeviceOrientationEvent.requestPermission=="function"){const e=document.createElement("button");e.style.cssText=`
                ...
                z-index: 10000;
                cursor: default;
            `,document.body.appendChild(e),e.addEventListener("click",()=>{DeviceOrientationEvent.requestPermission().then(i=>{i==="granted"&&this.enableDeviceOrientation(),e.remove()}).catch(()=>{e.remove()})}),e.click(),setTimeout(()=>{document.body.contains(e)&&e.remove()},5e3)}else this.enableDeviceOrientation()}enableDeviceOrientation(){let e,i=!1;const n=t=>{(t.alpha!==null||t.beta!==null||t.gamma!==null)&&(i=!0,clearTimeout(e),window.removeEventListener("deviceorientation",n),this.hasDeviceOrientation=!0,this.deviceControls?(this.deviceControls.disconnect(),this.deviceControls.connect(),this.deviceControls.calibrate()):this.deviceControls=new I(this.camera),this.controls&&(this.controls.enabled=!1))};window.addEventListener("deviceorientation",n),e=setTimeout(()=>{i||(window.removeEventListener("deviceorientation",n),this.controls&&(this.controls.enabled=!0))},2e3)}showFallbackMessage(){}createVignette(){const e=new O(2,2),i=new R({uniforms:{color:{value:new z(0)},opacity:{value:.5}},vertexShader:`
                varying vec2 vUv;
                void main() {
                    vUv = uv;
            ...
            `,transparent:!0});this.vignetteMesh=new p(e,i),this.vignetteMesh.position.z=0,this.scene.add(this.vignetteMesh)}loadPanorama(e){if(console.log("Loading panorama:",e),!e||typeof e!="object"){console.error("Invalid scene data:",e);return}if(e.type==="cube")try{this.renderer.domElement.style.display="block",this.renderer.domElement.style.zIndex="10",console.log("渲染器已设置为可见，z-index:",this.renderer.domElement.style.zIndex);const i=new q,n=[e.images.right,e.images.left,e.images.top,e.images.bottom,e.images.front,e.images.back];console.log("Loading cube textures:",n);const t=i.load(n,()=>{console.log("Cube textures loaded successfully"),this.renderer.domElement.style.display="block"},r=>{console.log("Loading progress:",r.loaded/r.total*100+"%")},r=>{console.error("Error loading cube textures:",r)});this.scene.background=t;const a=new j(500,60,40);a.scale(-1,1,1);const s=new w({envMap:t,side:N}),o=new p(a,s);for(;this.scene.children.length>0;)this.scene.remove(this.scene.children[0]);this.scene.add(o),console.log("全景网格已添加到场景"),this.camera.position.set(0,0,.1),this.controls.reset(),e.sceneName&&this.addEvidenceMarkers(e.sceneName)}catch(i){console.error("Error in loadPanorama:",i)}else console.error("Invalid panorama type:",e.type)}onWindowResize(){this.camera.aspect=window.innerWidth/window.innerHeight,this.camera.updateProjectionMatrix(),this.renderer.setSize(window.innerWidth,window.innerHeight)}animate(){requestAnimationFrame(this.animate.bind(this));const e=Date.now()*.001;this.markers.forEach(i=>{if(i.lookAt(this.camera.position),i.userData.glowEffect){const n=i.userData.glowEffect;n.lookAt(this.camera.position);const t=i.userData.pulseStartTime||0,a=Math.sin((e*2+t)*.5)*.5+.5,s=1+a*.3;n.scale.set(i.scale.x*s,i.scale.y*s,i.scale.z*s),n.material.opacity=.3+a*.2}i.userData.rings&&i.userData.rings.forEach((n,t)=>{n.lookAt(this.camera.position);const a=e+n.userData.offset,s=n.userData.initialScale*(1+a*n.userData.speed%1);n.scale.set(s,s,s),n.material.opacity=.5*(1-a*n.userData.speed%1)})}),this.isMobile&&this.hasDeviceOrientation&&this.deviceControls?this.deviceControls.update():this.controls&&this.controls.update(),this.renderer.render(this.scene,this.camera)}switchScene(e){console.log("Switching to panorama scene:",e),this.renderer&&this.renderer.domElement&&(this.renderer.domElement.style.display="block",this.renderer.domElement.style.zIndex="10",console.log("渲染器在场景切换时立即设置为可见, z-index:",this.renderer.domElement.style.zIndex));const i=document.querySelector(".background-image");i&&(i.style.opacity="0"),this.loadPanorama(e),e&&e.sceneName?(console.log("Adding evidence markers for scene:",e.sceneName),this.addEvidenceMarkers(e.sceneName)):console.warn("No sceneName provided in sceneData"),this.isMobile&&!this.hasDeviceOrientation&&this.requestDeviceOrientationPermission()}dispose(){this.renderer.dispose(),this.renderer.domElement.parentNode&&this.renderer.domElement.parentNode.removeChild(this.renderer.domElement)}createMarker(e,i){const n=new T(1,32),t=new w({color:16777215,side:b,transparent:!0,opacity:.9}),a=new p(n,t);return a.position.copy(e),a.scale.set(5,5,5),a.lookAt(this.camera.position),a.userData=i,this.scene.add(a),this.markers.push(a),this.addRingEffect(a),this.addPulseEffect(a),a}addRingEffect(e){for(let i=0;i<3;i++){const n=new B(1,1.2,32),t=new w({color:16777215,side:b,transparent:!0,opacity:.5}),a=new p(n,t);a.position.copy(e.position),a.scale.set(e.scale.x,e.scale.y,e.scale.z),a.lookAt(this.camera.position),a.userData={initialScale:e.scale.x,offset:i*2*Math.PI/3,speed:.5},this.scene.add(a),e.userData.rings=e.userData.rings||[],e.userData.rings.push(a)}}addPulseEffect(e){const i=new T(1.2,32),n=new w({color:16777215,transparent:!0,opacity:.5,side:b}),t=new p(i,n);t.position.copy(e.position),t.scale.set(e.scale.x,e.scale.y,e.scale.z),t.lookAt(this.camera.position),this.scene.add(t),e.userData.glowEffect=t,e.userData.pulseStartTime=Math.random()*2e3}onMarkerClick(e){const i=this.renderer.domElement.getBoundingClientRect();this.mouse.x=(e.clientX-i.left)/i.width*2-1,this.mouse.y=-((e.clientY-i.top)/i.height)*2+1,this.raycaster.setFromCamera(this.mouse,this.camera);const n=this.raycaster.intersectObjects(this.markers);if(n.length>0){const t=n[0].object;this.showEvidencePopup(t.userData)}}showEvidencePopup(e){var c,C;const i=document.querySelector(".evidence-popup");i&&i.remove();const t=((c=window.evidenceData)==null?void 0:c.en.find(h=>h.name===e.name||h.name===e.name.replace(" (Investigated)","")))||e,a=t.investigated||!1,s=document.createElement("div");s.className="evidence-popup",s.style.cssText=`
            position: fixed;
            top: 50%;
            left: 50%;
        ...
        `;const o=t.investigationCost||{type:"tech",amount:3},r=!a&&((C=window.detectiveManager)==null?void 0:C.hasEnoughPoints(o.type.toLowerCase(),o.amount));s.innerHTML=`
            <div style="position: relative;">
                ...
                <h3 style="margin: 0 0 15px 0; color: #ffffff;">${t.name}</h3>
                <p style="margin: 0 0 20px 0; line-height: 1.5;">${t.description}</p>
                ...
                ${a?`
                    ...
                    ">
                        ...
                        <p style="margin: 0; line-height: 1.5;">${t.detailedInfo}</p>
                        ${t.investigationDetails?`
                            <div style="margin-top: 10px; font-size: 12px; color: #888;">
                                <p>Investigated by: ${t.investigationDetails.investigatedBy}</p>
                                <p>Date: ${new Date(t.investigationDetails.investigatedAt).toLocaleString()}</p>
                                <p>Cost: ${t.investigationDetails.cost.amount} ${t.investigationDetails.cost.type} Points</p>
                            </div>
                        `:""}
                    </div>
                `:`
        ...
        `;const l=s.querySelector(".close-popup-btn");l.onclick=()=>s.remove();const m=h=>{s.contains(h.target)||(s.remove(),document.removeEventListener("click",m))};if(setTimeout(()=>{document.addEventListener("click",m)},0),!a){const h=s.querySelector(".add-evidence-btn");if(h.onclick=()=>{s.remove(),this.addToEvidence(t),this.showNotification("Evidence added: "+t.name),this.animateEvidenceButton()},t.investigationCost){const k=s.querySelector(".investigate-btn");r?k.onclick=()=>{if(!window.detectiveManager){this.showNotification("Detective system not initialized!");return}const v=o.type.toLowerCase();window.detectiveManager.modifyPoints(v,-o.amount);const y={...t,investigated:!0,investigationDetails:{cost:o,investigatedBy:window.detectiveManager.detectives[window.detectiveManager.currentDetective].name,investigatedAt:window.detectiveManager.getFormattedGameTime(),score:t.scoreValue}};this.addToEvidence(y),this.animateEvidenceButton(),window.detectiveManager&&window.detectiveManager.addEvidenceScore(t.scoreValue,t),s.remove(),this.showEvidencePopup(y),this.showNotification(`Investigation completed! Evidence Score: +${t.scoreValue}`)}:k.onclick=()=>{const v=window.detectiveManager.detectives[window.detectiveManager.currentDetective],y=o.type.toLowerCase()==="tech"?v.techPoints:v.labPoints;this.showNotification(`Insufficient points! Required ${o.amount} ${o.type} Points (Current: ${y})`)}}}document.body.appendChild(s)}addToEvidence(e){let i=!1,n=0;if(window.evidenceData&&window.evidenceData.en){for(const t of window.evidenceData.en)if(t.id>n&&(n=t.id),t.name===e.name||t.name===e.name.replace(" (Investigated)","")){i=!0,e.investigated&&(Object.assign(t,{investigated:!0,investigationDetails:e.investigationDetails,detailedInfo:e.detailedInfo}),console.log("Updated existing evidence with investigation results:",t));break}}if(!i&&window.evidenceData&&window.evidenceData.en){const t={id:n+1,name:e.name.replace(" (Investigated)",""),description:e.description,investigated:e.investigated||!1,investigationDetails:e.investigationDetails||null,detailedInfo:e.detailedInfo||null,scene:e.scene||null,investigationCost:e.investigationCost||null};window.evidenceData.en.push(t),console.log("Added new evidence:",t)}window.game&&window.game.evidencePage&&(window.game.evidencePage.setupEvidenceItems(),console.log("Evidence page updated"))}animateEvidenceButton(){const e=document.getElementById("evidence");if(!e)return;e.classList.add("evidence-btn-pulse");const i=document.createElement("style");i.innerHTML=`
            .evidence-btn-pulse {
        ...
        `,document.head.appendChild(i),setTimeout(()=>{e.classList.remove("evidence-btn-pulse")},1500)}showNotification(e){const i={"点数不足！需要":"Insufficient points! Required","Tech Points (当前:":"Tech Points (Current:","Lab Points (当前:":"Lab Points (Current:","已添加证物:":"Evidence added:","Detective system not initialized!":"Detective system not initialized!"};let n=e;for(const[s,o]of Object.entries(i))n=n.replace(s,o);const t=document.createElement("div");t.className="evidence-notification",t.style.cssText=`
            position: fixed;
            top: 20px;
            left: 50%;
            transform: translateX(-50%);
            ...
            min-width: 200px;
            max-width: 400px;
        `,t.innerHTML=n;const a=document.createElement("style");a.innerHTML=`
            @keyframes slideDown {
...
[Many messages have been deleted]`,scoreValue:25}},{position:new d(150,30,-100),data:{name:"Concert Group Photo",description:"A group photo taken at a concert, showing Linna, Join, and Mark",image:"./images/evidence/concert_photo.jpg",scene:"linnaroom",investigationCost:{type:"tech",amount:2},detailedInfo:"The photo was taken one year ago, showing Linna, Join, and Mark together at a concert. The photo was taken by Tom.",scoreValue:20}}]}[e]||[];this.markers.forEach(t=>{t.userData.glowEffect&&this.scene.remove(t.userData.glowEffect),this.scene.remove(t)}),this.markers=[],n.forEach(t=>{this.createMarker(t.position,t.data)}),console.log(`Added ${n.length} evidence markers in ${e}`)}}document.addEventListener("DOMContentLoaded",function(){console.log("DOM loaded, initializing PanoramaScene..."),setTimeout(function(){window.panoramaScene=new F,console.log("PanoramaScene created as global instance")},500)});document.addEventListener("DOMContentLoaded",()=>{if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)||window.innerWidth<=768){console.log("Mobile device detected, skipping instructions");const n=document.querySelector(".background-image");if(n){n.style.opacity="0",n.style.backgroundImage="url('./images/scene0.png')";const t=new Image;t.onload=()=>{n.style.opacity="1",n.style.transition="opacity 0.3s ease",setTimeout(()=>{if(window.gameTimer){window.gameTimer.hasCompletedStickers=!0,window.gameTimer.missionStartTime=new Date().getTime(),window.gameTimer.initTimer(),window.gameTimer.isRunning=!0,document.querySelectorAll("#timer, #mission, #mission-timer").forEach(r=>{r&&(r.style.color="#4CAF50",r.style.textShadow="0 0 10px rgba(76, 175, 80, 0.7)")}),document.querySelectorAll(".timer-label").forEach(r=>{r&&(r.style.color="#4CAF50")});const o=setInterval(()=>{window.panoramaScene&&(clearInterval(o),console.log("全景场景实例已就绪，切换到Mark房间"),window.gameTimer.switchToScene(window.gameTimer.SCENE_MARK_ROOM))},100);setTimeout(()=>{clearInterval(o),console.log("全景场景加载超时，使用默认场景")},5e3)}},1e3)},t.src="./images/scene0.png"}return}console.log("Desktop device detected - creating stickers");const e=document.querySelector(".background-image");if(e){e.style.opacity="0",e.style.backgroundImage="url('./images/scene0.png')";const n=new Image;n.onload=()=>{e.style.opacity="1",e.style.transition="opacity 0.3s ease",i()},n.src="./images/scene0.png"}else i();function i(){if(document.querySelectorAll("#timer, #mission, #mission-timer").forEach(r=>{r&&(r.style.color="#888888")}),document.querySelector(".sticker-container")){console.log("Sticker container already exists, skipping creation");return}const t=document.createElement("div");t.className="sticker-container",t.style.position="fixed",t.style.width="100%",t.style.height="100vh",t.style.zIndex="200",t.style.pointerEvents="none",t.style.opacity="0",document.body.appendChild(t),requestAnimationFrame(()=>{requestAnimationFrame(()=>{t.style.transition="opacity 0.3s ease",t.style.opacity="1"})}),console.log("Creating initial stickers, count:",5),[{text:"YOU!: Here is who you are and what's your special skill, you probably know some of you already.",position:{top:"10%",left:"10%"},rotation:"-3deg",color:"rgba(252, 246, 231, 0.92)",number:1},{text:"TIME!: Time is everything, solve it before nothing too late! and Check your EVENTS in bottom right corner!",position:{top:"10%",left:"70%"},rotation:"2deg",color:"rgba(250, 247, 235, 0.92)",number:2},{text:"POINTS!: You will get points for good clue you find, Manage them nicely.",position:{top:"60%",left:"12%"},rotation:"-2deg",color:"rgba(248, 245, 235, 0.92)",number:4},{text:"SCENE!: No detective is complete without a scene, here is the scenes where you need to find the clues.",position:{top:"40%",left:"80%"},rotation:"4deg",color:"rgba(255, 250, 240, 0.92)",number:3},{text:"BULABULA! The first 3 buttons are the tools you need to keep working on, Hit the big red button when really finished.",position:{top:"65%",left:"60%"},rotation:"1deg",color:"rgba(251, 248, 237, 0.92)",number:5}].forEach(r=>{const l=document.createElement("div");l.className="sticker",l.style.position="absolute",l.style.width="230px",l.style.height="230px",l.style.backgroundColor=r.color,l.style.border="1px solid rgba(0,0,0,0.08)",l.style.borderRadius="2px",l.style.boxShadow=`
            ...
            `,l.style.padding="15px",l.style.textAlign="center",l.style.display="flex",l.style.flexDirection="column",l.style.justifyContent="space-between",l.style.alignItems="center",l.style.top=r.position.top,l.style.left=r.position.left,l.style.transform=`rotate(${r.rotation})`,l.style.transition="all 0.3s ease",l.style.cursor="default",l.style.zIndex="200",l.style.pointerEvents="auto";const m=document.createElement("div");m.textContent=r.number,m.style.fontSize="48px",m.style.fontWeight="bold",m.style.color="#333",m.style.position="absolute",m.style.top="10px",m.style.left="10px",m.style.textShadow="2px 2px 4px rgba(0,0,0,0.1)",l.appendChild(m);const c=document.createElement("p");c.textContent=r.text,c.style.margin="0",c.style.fontSize="18px",c.style.lineHeight="1.4",c.style.color="#555",c.style.paddingLeft="20px",c.style.paddingTop="10px",c.style.textAlign="left",l.appendChild(c),t.appendChild(l),l.onmouseover=()=>{l.style.boxShadow=`
                ...
                `,document.body.appendChild(this.eventsModal),this.eventsModal.querySelector(".close").addEventListener("click",()=>{this.eventsModal.style.display="none"}),window.addEventListener("click",n=>{n.target===this.eventsModal&&(this.eventsModal.style.display="none")}),this.eventsList=this.eventsModal.querySelector(".events-list")),this.eventsList.innerHTML="";const i=this.events.filter(n=>n.triggered);if(i.length===0?this.eventsList.innerHTML='<div class="event-item">No events yet.</div>':i.forEach(n=>{const t=document.createElement("div");t.className=`event-item ${n.read?"":"unread"}`;const a=n.image||"./images/events/placeholder.gif",s=e&&e===n||this.lastTriggeredEvent&&this.lastTriggeredEvent===n;t.innerHTML=`
                        <div class="event-header" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center;">
                            <div style="flex-grow: 1;" translate="yes" lang="en">
                                <span class="event-name">${n.name}</span>
                                <span class="event-time">Time: ${n.triggerTime||"00:00:00"}</span>
                            </div>
                    ...
                    `;const o=t.querySelector(".event-image-container");this.optimizedImageDisplay(a,o),t.querySelector(".event-header").addEventListener("click",()=>{const l=t.querySelector(".event-description"),m=t.querySelector(".toggle-description");l.style.display=l.style.display==="none"?"flex":"none",m.textContent=m.textContent==="▼"?"▲":"▼",n.read=!0,t.classList.remove("unread"),this.updateEventNotification()}),this.eventsList.appendChild(t),e&&e===n&&(n.read=!0,t.classList.remove("unread"),this.updateEventNotification())}),this.eventsModal.style.display="block",e){const n=this.eventsList.querySelectorAll(".event-item"),t=i.findIndex(a=>a===e);t>=0&&n[t]&&n[t].scrollIntoView({behavior:"smooth",block:"start"})}}catch(i){console.error("Error showing events:",i);const n=document.getElementById("modal");if(n){const t=document.getElementById("modal-title"),a=document.getElementById("modal-body");if(t&&a){t.textContent="Events";let s='<div class="events-container" style="max-height: 90vh; overflow-y: auto;">';const o=this.events.filter(r=>r.triggered);o.length===0?s+='<div class="event-item">No events yet.</div>':o.forEach(r=>{const l=r.image||"./images/events/placeholder.gif";s+=`
                            ...
                            `,r.read=!0}),s+="</div>",a.innerHTML=s,n.style.display="block",this.updateEventNotification()}}else alert("Events feature is currently unavailable. Please try again later.")}}addEventToList(e){if(!this.eventsModal||!this.eventsList)return;const i=document.createElement("div");i.className=`event-item ${e.read?"":"unread"}`;const n=e.image||"./images/events/placeholder.gif",t=e===this.lastTriggeredEvent;i.innerHTML=`
            <div class="event-header" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center;">
                <div style="flex-grow: 1;" translate="yes" lang="en">
                    <span class="event-name">${e.name}</span>
                    <span class="event-time">Time: ${e.triggerTime||"00:00:00"}</span>
                </div>
                <button class="toggle-description" style="pointer-events: none;">${t?"▲":"▼"}</button>
            </div>
            <div class="event-description" style="display: ${t?"flex":"none"}; background-color: rgba(50, 50, 50, 0.5) !important; flex-direction: row; flex-wrap: wrap;">
                <div class="event-image-container" style="width: 40%; flex-shrink: 0; margin-right: 15px; margin-bottom: 10px;"></div>
                <div class="event-text" style="flex: 1; min-width: 300px;" translate="yes" lang="en">
                    ${e.description}
                </div>
            </div>
        `;const a=i.querySelector(".event-image-container");this.optimizedImageDisplay(n,a),i.querySelector(".event-header").addEventListener("click",()=>{const o=i.querySelector(".event-description"),r=i.querySelector(".toggle-description");o.style.display=o.style.display==="none"?"flex":"none",r.textContent=r.textContent==="▼"?"▲":"▼",e.read=!0,i.classList.remove("unread"),this.updateEventNotification()}),this.eventsList.appendChild(i),t&&i.scrollIntoView({behavior:"smooth",block:"start"})}}window.EventManager=_;document.addEventListener("DOMContentLoaded",()=>{console.log("EventManager script loaded");const g=document.createElement("style");g.textContent=`
        #events-modal.modal {
